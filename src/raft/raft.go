	...
	if reply.Term == rf.currentTerm && rf.role == Leader {
		...
		if reply.XTerm == -1 {
			...
			if rf.lastIncludedIndex >= reply.XLen {
				// 由于snapshot被截断
				// lab4B Modify
				//这里不直接添加InstallSnapshot的处理，
				// 而是令 rf.nextIndex[serverTo] = rf.lastIncludedIndex
				// 这样之后在给serverTo发送的心跳中，会更新PrevLogIndex = nextIndex - 1
				// 会有 PrevLogIndex < lastIncludedIndex 然后会在心跳函数中触发 handleInstallSnapshot
