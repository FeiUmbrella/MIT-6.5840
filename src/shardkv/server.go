...
func (kv *ShardKV) Execute(command Command, reply *CommandReply) {
	...
	case result := <-notifyChan:
		if startTerm == result.AppliedTerm {
	...
	if shardsInfo.ConfigNum == kv.currentConfig.Num {
		...
		for shardID, shardData := range shardsInfo.Shards {
			...
			if shard.Status == Pulling {
				...
				// todo:这里直接break吗？不应该遍历所有的shardID，让状态为Pulling的数据copy吗
				// A: 这个server的status应该是Pulling但此时不是，说明已经应用过，这个request是个重复的request
				break
			}
		}
		...
		// todo: 这里正常运行，没有任何问题为什么不用设置 reply.Err = OK ?
		// 在 applyDeleteShards 中一定会返回 OK， 如果每次一个 applyInsertShard后面紧跟一个 applyDeleteShards的话就河里
	} else {
...
func (kv *ShardKV) needSnapshot() bool {
	return kv.maxRaftState != -1 && kv.persister.RaftStateSize() >= kv.maxRaftState*95/100
}
...
func (kv *ShardKV) configurationAction() {
	...
	if canPerformNextConfig {
		...
		// 当sc中的Config少于currentConfigNum + 1时，只会返回最新的Config，此时的ConfigNum仍小于currentConfigNum + 1
		...
		go func(servers []string, configNum int, shardIDs []int) {
			...
			for _, server := range servers {
				...
				if srv.Call("ShardKV.DeleteShardsData", &gcTaskArgs, gcTaskReply) && gcTaskReply.Err == OK {
					// 如果gc.TaskArgs.configNum != kv.currentConfig.Num 在Execute传递到applyDeleteShards的时候也不会执行
